...
function createGenerateBoard() {
  ...
  // );
  let count = 0;
  return function (numberOfRows: number, numberOfCols: number) {
    count++;
    const board = Array.from({ length: numberOfRows }, () =>
      Array.from({ length: numberOfCols }, () => false)
    );
    if (count == 1) {
      // board[5][5] = true;
...
function renderBoard(board: boolean[][]) {
...
function setStateCell(
  ...
  if (!board[row][cell] && surroundings.length == 3) {
    ...
    (board[row][cell] && surroundings.length == 2) ||
...
function checkCell(board: boolean[][], row: number, cell: number) {
  const surroundings = [];
  for (let i = -1; i <= 1; i++) {
    for (let j = -1; j <= 1; j++) {
      if (i == 0 && j == 0) continue;
...
function spawnGeneration(board: boolean[][]) {
